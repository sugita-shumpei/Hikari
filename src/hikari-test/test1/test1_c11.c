...
int main() {
	...
	HK_PLUGIN_MANAGER_INIT_FUNCTION(pl_manager, HKPluginManager_load);
	HK_PLUGIN_MANAGER_INIT_FUNCTION(pl_manager, HKPluginManager_unload);
	HK_PLUGIN_MANAGER_INIT_FUNCTION(pl_manager, HKPluginManager_internal_getProcAddressFromPlugin);

	if (HKPluginManager_load(pl_manager, "D:\\Users\\shumpei\\Document\\CMake\\Hikari\\build\\src\\hikari\\Debug\\hikari-shape.dll")){
		HK_PLUGIN_MANAGER_INIT_FUNCTION_FROM_PLUGIN(pl_manager, (HK_OBJECT_TYPEID_PluginShape), HKObjMesh_create);
		HK_PLUGIN_MANAGER_INIT_FUNCTION_FROM_PLUGIN(pl_manager, (HK_OBJECT_TYPEID_PluginShape), HKObjMesh_setFilename);
		//HK_PLUGIN_MANAGER_INIT_FUNCTION(pl_manager, HKPluginManager_getDependedCount);
