...
#include <hikari/framework/core/transform.h>
#include <hikari/framework/core/tuple.h>
#include <hikari/framework/core/utils.h>

namespace hikari {
  namespace core {
    // 任意精度のPropertyを取得可能
    using PropertySignedTypes    = std::tuple<
      I8 ,//Char型, 文字を表す場合に使用, 単体では使用しない
      I16,// 基本的に使用しない,
      I32,// Int型, 整数を表す場合基本的にこの型を使用すること
      I64 //IPtr型, ポインター間の距離を表す場合基本的にこの型を使用すること
    >;
    using PropertyUnsignedTypes  = std::tuple<
      U8 ,//Byte型, バイトデータを表す場合基本的にこの型を使用すること
      U16,// 基本的に使用しない,
      U32,// 基本的に使用しない,
      U64 //UPtr型, ポインター,サイズを表す場合基本的にこの型を使用すること, 配列としてのみ使用する
    >;
    using PropertyFloatTypes     = std::tuple<
      F32,// Float型, 小数を表す場合基本的にこの型を使用すること
      F64 //Double型, どうしても倍精度が必要なときのみ使用
    >;
    using PropertyIntegerTypes   = typename concat_tuple<PropertySignedTypes, PropertyUnsignedTypes>::type;
    using PropertyNumericTypes   = typename concat_tuple<PropertySignedTypes, PropertyUnsignedTypes, PropertyFloatTypes, std::tuple<Bool>>::type;
