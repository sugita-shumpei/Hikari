  ...
  inline namespace core {
    ...
    struct Transform {
      ...
      auto getJSONString() const ->std::string {
        ...
        res += "{ ";
        res += " \"type\" : \"Transform\", ";
        if (m_data.index() == (int)TransformType::eMat) {
          res += " \"matrix\" : " + convertToString(std::get<(int)TransformType::eMat>(m_data)) + " ";
        }
        else {
          auto& data = std::get<(int)TransformType::eTRS>(m_data);
          res += " \"position\" : " + convertToString(data.position) + ", ";
          res += " \"rotation\" : " + convertToJSONString(data.rotation) + ", ";
          res += " \"scale\" : "    + convertToString(data.scale) + " ";
        }
        res += "}";
      }
    ...
    template<> struct Type2String<Transform> { static inline constexpr char value[] = "Transform"; static inline constexpr size_t len = sizeof(value) - 1; };
    inline auto convertToString(const Transform& v) -> Str {
      if (v.getType() == TransformType::eTRS) {
        auto data = v.getTRS();
        std::string res = "{";
        res += " \"position\" : " + convertToString(data->position)     + ", ";
        res += " \"rotation\" : " + convertToJSONString(data->rotation) + ", ";
        ...
        auto data = v.getMat();
        std::string res = "{ \"matrix\" : " + convertToString(data) + " }";
        return res;
      }

    }

    auto convertStringToTransform(const Str& v) -> Option<Transform>;
    HK_CONVERT_FROM_STRING_DEFINE(Transform);

    inline auto convertToJSONString(const Transform& v) -> Str {
      if (v.getType() == TransformType::eTRS) {
        auto data = v.getTRS();
        std::string res = "{ \"type\" : \"Transform\", ";
        res += " \"position\" : " + convertToString(data->position) + ", ";
        res += " \"rotation\" : " + convertToJSONString(data->rotation) + ", ";
        res += " \"scale\" : " + convertToString(data->scale) + " ";
        res += "}";
        ...
        auto data = v.getMat();
        std::string res = "{ \"type\" : \"Transform\", \"matrix\" : " + convertToString(data) + " }";
        return res;
      }
    ...
    HK_CONVERT_JSON_STRING_TO_DEFINE(Transform);
  }
}
